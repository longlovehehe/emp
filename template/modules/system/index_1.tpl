...
<div class="userinfo">
        ...
        <div class="uname">姓名：{$smarty.session.eown.om_id}</div>
        <ul class="list admintype">
                <li><label class='title'>管理员级别：</label>{$smarty.session.eown.om_id|level}</li>
                <li title='{$smarty.session.eown.om_area|mod_area_name}'><label class='title'>管辖区域：</label><span class='ellipsis2' style='width: 240px;'>{$smarty.session.eown.om_area|mod_area_name}</span></li>
                <li><label class='title'>管辖企业：</label>{$en}</li>
                <li><label class='title'>管辖设备：</label>{$device}</li>
        </ul>
        <!--帐号状态-->
        <ul class="list logininfo">
                <li>上次登录地址：{$smarty.session.eown.om_lastlogin_ip}</li>
                <li>上次登录时间：{$smarty.session.eown.om_lastlogin_time}</li>
        </ul>
</div>
<div class="anbbs">
