...
<form id="form" class="base mrbt10" action="?m=enterprise&a=users_save_shell">
    ...
    <div class="block sw user">
        <input type="hidden" name="u_p_function" value="{$item.u_p_function|escape:"html"}">
        <div class="block" style="float:left"><label class="title" style="">{"增值功能"|L}：</label></div>
        <div class="title" style="width:220px; height: auto; border:1px solid #ccc; padding: 10px;"><div id="product_select" class="autofix  autocheck" readonly disabled="true" value="{$item.u_p_function|escape:"html"}" action="?m=product&a=ip_option&e_id={$data.e_id}"></div></div>
        {*<select  name="u_product_id[]" id="selectprice" multiple="TRUE" value="{$item.u_product_id|escape:"html"}" class="autofix  autoeditselect " action="?m=product&a=p_option&e_id={$data.e_id}" >
            <option value="">{"无"|L}</option>
        </select>*}
        {*<span>{"价格"}￥<span id="price">0</span></span>*}
    </div>
    <div class="block">
    ...
    <div class="block radio" value="{$item.u_attr_type}">
        <div class="line {if $item.u_attr_type eq 0}{if $data.do eq edit}none{/if}{/if}">
            ...
            <label class="radiotext">
                <input autocomplete="off"  value="1" name="u_attr_type" type="radio" {if $data.do eq edit} {if $item.u_attr_type eq 1}checked="checked" disabled="disabled"{/if}{/if}/>
                <span>{"测试"|L}</span>
            </label>
            <label class="radiotext">
                <input autocomplete="off"  value="0" name="u_attr_type" type="radio"   {if $data.do eq edit} {if $item.u_attr_type eq 1}disabled="disabled"{/if}{/if}/>
                <span>{"商用"|L}</span>
            </label>
            {if $data.do eq edit} {if $item.u_attr_type eq 1}<input type="hidden" name="u_attr_type" value="1"/>{/if}{/if}
        </div>
    </div>
...
{*            <input autocomplete="off"   maxlength="32" value="{$item.u_terminal_type}" name="u_terminal_type" type="text"  />*}
    ...
    <div class="auto_config {if $item.u_auto_config == 0}hide{/if} ">
        ...
        <div class="sw user block">
            ...
            <input autocomplete="off" class="u_imei"  maxlength="15" value="{$item.u_imei}" name="u_imei" u_imei="true" type="text" />
        </div>
        <div class="sw user block">
